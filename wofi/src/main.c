...
static void print_usage(char** argv) {
	...
	printf("--fork-noclose\t\t-F\tForks the menu without closing std descriptors\n");
...
int main(int argc, char** argv) {
	...
	int opt;
	while((opt = getopt_long(argc, argv, "hfFc:s:C:dS:W:H:p:x:y:nImk:t:P::eEbM:iqvl:aD:L:w:O:GQ:o:r:", opts, NULL)) != -1) {
		switch(opt) {
		case 'h':
			print_usage(argv);
			break;
