...
local get_findApp = ya.sync(function(st)
	return st.app
end)
...
function M:entry()
	...
	local child, err =
		Command(shell):arg("-c"):arg(cmd):stdin(Command.INHERIT):stdout(Command.PIPED):stderr(Command.INHERIT):spawn()
