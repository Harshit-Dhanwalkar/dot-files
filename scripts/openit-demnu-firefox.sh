...
add_index_numbers() {
    ...
    while IFS= read -r line; do
        ...
        case "$clean_line" in
            "Back"|"Back/"|"Exit"|"Back/Exit"|"Back / Exit")
                printf "%s\n" "$clean_line"
                ;;
            *)
                printf "%2d. %s\n" "$count" "$clean_line"
...
append_icon() {
    ...
    case "$url" in
        ...
        *bitbucket.org*)
            echo "  $url" ;;    # Bitbucket
        *stackoverflow.com*)
            echo "  $url" ;;    # StackOverflow
        *stackexchange.com*)
            echo "  $url" ;;    # StackExchange
        *wikipedia.org*)
            echo "󰖬  $url" ;;    # Wikipedia
        *reddit.com*)
            echo "  $url" ;;    # Reddit
        ...
        *fedora.org*)
            echo "  $url" ;;    # Fedora
        https://*|http://*)
            echo "󰌷  $url" ;;    # Fallback icon for any generic URL
        *)
            echo "$url" ;;       # If no match, print raw URL
    esac
}
...
while true; do
    ...
    MENU_CATEGORIES=$(printf "Exit\n%s\n" "$CATEGORIES")

    SELECTED_CATEGORY=$(
        add_index_numbers "$MENU_CATEGORIES" |
        $DMENU_CMD -l $(calculate_lines "$MENU_CATEGORIES") -p "Category:" |
        remove_index_numbers
    )

    # Back or ESC
    [[ -z "$SELECTED_CATEGORY" || "$SELECTED_CATEGORY" == "Exit" ]] && exit 0

    # --- URL Menu Loop ---
    while true; do
        CATEGORY_URLS=$(echo "$ALL_LINKS" |
            grep "^${SELECTED_CATEGORY}:" |
            sed "s/^${SELECTED_CATEGORY}:[ \t]*//")

        # Add icons to URL list
        ICON_URLS=""
        while IFS= read -r u; do
            ICON_URLS+=$(append_icon "$u")$'\n'
        done <<< "$CATEGORY_URLS"

        MENU_URLS=$(printf "Back\n%s\n" "$ICON_URLS")

        SELECTED_URL=$(
            add_index_numbers "$MENU_URLS" |
            $DMENU_CMD -l $(calculate_lines "$MENU_URLS") -p "$SELECTED_CATEGORY:" |
            remove_index_numbers
        )

        # User pressed ESC or chose Back → go to category menu
        [[ -z "$SELECTED_URL" || "$SELECTED_URL" == "Back" ]] && break

        # Remove icon before launching
        CLEAN_URL=$(echo "$SELECTED_URL" | sed 's/^[^ ]*  //')

        launch_url "$CLEAN_URL"
    done
done
