...
def format_compact(val, suffix=""):
    ...
    val /= 1024
    if val < 1024:
        return f"{val:.1f}K{suffix}"
    val /= 1024
    if val < 1024:
        return f"{val:.1f}M{suffix}"
    val /= 1024
...
def get_drives():
    ...
    for p in psutil.disk_partitions():
        if any(x in p.mountpoint for x in ['/snap', '/boot', '/docker', '/var', '/run', '/sys', '/proc', '/dev']):
            continue
