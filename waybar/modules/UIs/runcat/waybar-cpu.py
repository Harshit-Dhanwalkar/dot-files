...
def main():
    try:
        ...
        try:
            ...
            for label in ["coretemp", "k10temp", "zenpower"]:
                if label in temps:
        ...
        def get_temp_color(temp):
            ...
            elif temp < 65: return "#ffff00"
            elif temp < 75: return "#ff9900"
            else: return "#ff0000"
        # Simple output
        output = {
            "text": f" <span foreground='{get_temp_color(max_cpu_temp)}'>{max_cpu_temp}°C</span>",
            "tooltip": f"CPU Usage: {cpu_percent:.1f}%\nTemperature: {max_cpu_temp}°C",
            "class": "cpu",
            "alt": f"{cpu_percent:.0f}%"
        }
...
#
# # TERMINAL = os.environ.get("TERMINAL") or shutil.which("kitty") or "xterm"
...
#     print(json.dumps(output))
# except Exception as e:
#     # Print error to stderr so Waybar can see it
#     print(f"ERROR: {str(e)}", file=sys.stderr)
#     traceback.print_exc(file=sys.stderr)
#     # Fallback output so Waybar doesn't crash
#     print(json.dumps({
#         "text": " ERROR",
#         "class": "error"
#     }))
